...
import type { BucketedIndex, CityRecord } from './types.js'
...
function readTextFileMaybeZip(filePath: string): string {
...
function parseCountryInfo(filePath: string): Map<string, string> {
...
function parseAdmin1(filePath: string): Map<string, string> {
  const text = fs.readFileSync(filePath, 'utf8')
  const map = new Map<string, string>()
...
function parseCities(citiesPath: string, countryMap: Map<string, string>, admin1Map: Map<string, string>, cellSizeDeg: number): BucketedIndex {
  const text = readTextFileMaybeZip(citiesPath)
  const buckets: Record<string, CityRecord[]> = {}
...
export function buildIndex(options: BuildOptions): BucketedIndex {
...
if (process.argv[1] && process.argv[1].endsWith('build.js')) {
  ...
  if (!citiesPath || !countryInfoPath || !admin1Path || !outPath) {
    console.error(
      'Usage: node dist/build.js --cities=./data/cities15000.txt --countryInfo=./data/countryInfo.txt --admin1=./data/admin1CodesASCII.txt --out=./data/index.json [--cellSizeDeg=1]',
    )
    process.exit(1)
  }
